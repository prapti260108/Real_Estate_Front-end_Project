...
const ExploreProjects = () => {
  ...
  const normalizeImgUrl = (u) => {
    ...
    if (s.includes("uploads/")) {
      return s.startsWith("/") ? `https://real-estate-back-end-project-2.onrender.com${s}` : `https://real-estate-back-end-project-2.onrender.com/${s}`;
    }
    return `https://real-estate-back-end-project-2.onrender.com/uploads/${s.replace(/^\//, "")}`;
  };
      ...
      if (selectedBuilder) {
        ...
        if (decodedBuilderId) {
          (async () => {
            try {
              setLoading(true);
              const resp = await fetch("https://real-estate-back-end-project-2.onrender.com/api/search", {
                method: "POST",
